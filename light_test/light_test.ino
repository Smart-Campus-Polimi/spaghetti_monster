/* Grove - Light Sensor demo v1.0
* 
* Signal wire to A0.
* By: http://www.seeedstudio.com
*/
#include <math.h>

#define LIGHT_SENSOR A0//Grove - Light Sensor is connected to A0 of Arduino
#define LIGHT_SENSOR_2 A1//Grove - Light Sensor is connected to A0 of Arduino

//#define DUST_SENSOR A2
const int ledPin=3;                 //Connect the LED Grove module to Pin12, Digital 12
const int thresholdvalue=10;         //The treshold for which the LED should turn on. Setting it lower will make it go on at more light, higher for more darkness
float Rsensor; //Resistance of sensor in K
void setup() 
{
    pinMode(8,INPUT);
    Serial.begin(9600);                //Start the Serial connection
    pinMode(ledPin,INPUT);            //Set the LED on Digital 12 as an OUTPUT
   
}
void loop() 
{
    int dustValue = pulseIn(8, LOW);
    Serial.println("the dust data is: ");
    Serial.println(dustValue);
    int sensorValue = analogRead(LIGHT_SENSOR); 
    int sensorValue_2 = analogRead(LIGHT_SENSOR_2); 

    Rsensor = (float)(1023-sensorValue)*10/sensorValue;
    Serial.print("the analog read data is ");
    Serial.print(sensorValue);
    Serial.print(" , ");
    Serial.println(sensorValue_2);
    Serial.println("the sensor resistance is ");
    Serial.println(Rsensor,DEC);//show the ligth intensity on the serial monitor;
    delay(1000);
}
